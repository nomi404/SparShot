...
public class controllerballAi : MonoBehaviour
{
    ...
    private void Awake()
    {
        battleSystem = GameObject.FindGameObjectWithTag("battleSystemAi");
    ...
    void Start()
    {
        ...
        bs = battleSystem.GetComponent<battleSystemAi>();
    ...
    void OnMouseUp()
    {
        ...
        else if (startPos.y < 0 && belongsToPlayer1 == false)
        {
            ...
            if (transform.childCount > 0)
            {
                if (gameObject.GetComponentInChildren<dragAi>().poweredUp == true)
                {
                    Destroy(this.gameObject.transform.GetChild(0).gameObject);
                    Power = 10;
                }
            ...
            if (Input.GetMouseButtonUp(0))
            {
                ...
                helperScript.direction = direction;
